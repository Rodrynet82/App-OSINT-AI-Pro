...
// Sistema completo con login, dashboard, mapa Kaspersky, 19 pruebas, exportaci├│n PDF/JSON, y m├ís
...
const applicationData = {
  ...
  detailed_explanations: {
    analisis_ia: {
      title: "An├ílisis de Inteligencia Artificial",
      description: "Sistema avanzado de an├ílisis con IA que procesa m├ís de 50 fuentes de datos simult├íneamente",
      metrics: [
    ...
    investigaciones: {
      ...
      description: "Estado actual de investigaciones en curso con an├ílisis automatizado",
    ...
    score: {
      ...
      description: "Puntuaci├│n calculada basada en m├║ltiples factores de amenaza",
      components: [
        { factor: "Exposici├│n de Servicios", score: 85, weight: "30%" },
        { factor: "Vulnerabilidades Conocidas", score: 72, weight: "25%" },
        { factor: "Reputaci├│n de Dominio", score: 45, weight: "20%" },
        { factor: "Actividad Sospechosa", score: 58, weight: "25%" }
      ]
    },
    paises: {
      title: "Distribuci├│n Geogr├ífica",
      description: "An├ílisis de amenazas por ubicaci├│n geogr├ífica",
      countries: [
  ...
  notification_settings: [
    { id: "email_alerts", name: "Alertas por Email", enabled: true, frequency: "Inmediato", description: "Notificaciones cr├¡ticas por correo electr├│nico" },
    { id: "push_notifications", name: "Notificaciones Push", enabled: true, frequency: "Tiempo Real", description: "Alertas instant├íneas en el navegador" },
    ...
    { id: "report_ready", name: "Reportes Listos", enabled: false, frequency: "Diario", description: "Cuando los reportes est├ín listos para descarga" }
  ],
  api_configurations: [
...
const analysisTests = [
  { name: "WHOIS Lookup", description: "Informaci├│n de registro del dominio", category: "basic" },
  { name: "DNS Analysis", description: "An├ílisis de registros DNS", category: "basic" },
  { name: "IP Geolocation", description: "Localizaci├│n geogr├ífica", category: "basic" },
  { name: "Port Scanning", description: "Escaneo de puertos abiertos", category: "network" },
  { name: "SSL Certificate Check", description: "Verificaci├│n de certificados SSL", category: "security" },
  { name: "Web Technology Detection", description: "Detecci├│n de tecnolog├¡as web", category: "web" },
  { name: "Email Verification", description: "Verificaci├│n de direcciones email", category: "email" },
  { name: "Social Media Presence", description: "Presencia en redes sociales", category: "social" },
  { name: "Domain Reputation", description: "Reputaci├│n del dominio", category: "reputation" },
  { name: "Malware Detection", description: "Detecci├│n de malware", category: "security" },
  { name: "Phishing Analysis", description: "An├ílisis de phishing", category: "security" },
  ...
  { name: "Data Breach Check", description: "Verificaci├│n de filtraciones", category: "security" },
  { name: "Subdomain Enumeration", description: "Enumeraci├│n de subdominios", category: "recon" },
  { name: "HTTP Headers Analysis", description: "An├ílisis de headers HTTP", category: "web" },
  { name: "Open Source Intelligence", description: "Inteligencia de fuentes abiertas", category: "intelligence" },
  { name: "Network Topology", description: "Topolog├¡a de red", category: "network" },
  { name: "Behavioral Analysis", description: "An├ílisis de comportamiento", category: "advanced" }
];

// OSINT tools database ÔÇö expanded catalog
const toolsDatabase = {
  'An├ílisis de Red e IP': {
    icon: 'fas fa-network-wired',
    color: '#3B82F6',
    gradient: 'linear-gradient(135deg, #1d4ed8, #3b82f6)',
    description: 'Herramientas de an├ílisis de infraestructura y redes',
    tools: [
      {
        name: 'WHOIS',
        description: 'Obt├®n informaci├│n completa de registro de un dominio: titular, registrador, fechas de creaci├│n/expiraci├│n y servidores de nombres. Esencial para identificar la propiedad de un activo online.',
        shortDesc: 'Informaci├│n de registro del dominio',
        icon: 'fas fa-globe',
        endpoint: 'whois',
        form: {
          title: 'WHOIS Lookup',
      ...
      {
        name: 'DNS Lookup',
        description: 'Consulta todos los registros DNS de un dominio: A, AAAA, MX, NS, TXT, CNAME y SOA. Permite descubrir la infraestructura de email, servidores y pol├¡ticas de seguridad (SPF/DKIM).',
        shortDesc: 'An├ílisis completo de registros DNS',
        icon: 'fas fa-server',
        endpoint: 'dns',
        form: {
          title: 'DNS Lookup',
      ...
      {
        name: 'IP Geolocation',
        description: 'Localiza geogr├íficamente cualquier direcci├│n IP y obt├®n informaci├│n sobre el ISP, ASN, organizaci├│n propietaria y coordenadas aproximadas. ├Ütil para tracing y atribuci├│n de ataques.',
        shortDesc: 'Localizaci├│n geogr├ífica de IPs',
        icon: 'fas fa-map-marker-alt',
        endpoint: 'ipinfo',
        form: {
          title: 'IP Geolocation',
          fields: [
            { name: 'ip', type: 'text', label: 'Direcci├│n IP', placeholder: '8.8.8.8', required: true }
          ]
        }
      },
      {
        ...
        form: {
          ...
          fields: [
            ...
            { name: 'ports', type: 'text', label: 'Puertos', placeholder: '1-1000 ├│ 22,80,443', required: true }
          ]
        }
      },
      {
        name: 'SSL Checker',
        description: 'Analiza el certificado TLS/SSL de un dominio: emisor, validez, algoritmos de cifrado y vulnerabilidades conocidas (Heartbleed, POODLE). Fundamental para auditor├¡as de seguridad web.',
        shortDesc: 'An├ílisis de certificados SSL/TLS',
        icon: 'fas fa-lock',
        endpoint: null,
        form: {
          title: 'SSL Checker',
      ...
      {
        name: 'Traceroute',
        description: 'Traza la ruta de red entre tu host y el objetivo, identificando cada salto de router. Permite detectar puntos de fallo, latencia y proveedores de tr├ínsito utilizados en la infraestructura.',
      ...
      {
        name: 'Shodan Search',
        description: 'Busca dispositivos conectados indexados por Shodan: c├ímaras, routers, servidores industriales y m├ís. Descubre activos expuestos involuntariamente a internet por una organizaci├│n.',
        shortDesc: 'B├║squeda de dispositivos en Shodan',
  ...
  'Email & Domain Intelligence': {
    ...
    tools: [
      {
        name: 'Email Verifier',
        description: 'Verifica si una direcci├│n de email existe y es v├ílida sin necesidad de enviar un mensaje. Comprueba formato, dominio activo, registro MX y si el buz├│n est├í activo en el servidor de correo.',
        shortDesc: 'Verificaci├│n de existencia del email',
      ...
      {
        name: 'Breach Hunter',
        description: 'Comprueba si un email ha sido comprometido en filtraciones de datos conocidas (HIBP, DeHashed). Muestra en qu├® brechas aparece, qu├® datos fueron expuestos y cu├índo ocurri├│ la filtraci├│n.',
        shortDesc: 'B├║squeda en bases de datos filtradas',
      ...
      },
      {
        ...
        description: 'Eval├║a la reputaci├│n de un dominio en m├║ltiples motores antivirus y listas negras. Detecta si ha sido utilizado para phishing, malware o spam. Integrado con VirusTotal y otras fuentes.',
        shortDesc: 'Reputaci├│n y blacklists del dominio',
      ...
      {
        name: 'MX Records',
        description: 'Consulta espec├¡ficamente los registros MX para identificar el proveedor de correo (Google Workspace, Office 365, ProtonMail...). Revela si el dominio utiliza un relay de seguridad o filtro antispam.',
        shortDesc: 'An├ílisis de configuraci├│n de correo',
        icon: 'fas fa-at',
        endpoint: 'dns',
        form: {
          title: 'MX Records',
      ...
      {
        name: 'SPF/DKIM Check',
        description: 'Audita las pol├¡ticas de autenticaci├│n de correo de un dominio: registros SPF (remitentes autorizados), DKIM (firma criptogr├ífica) y DMARC (pol├¡tica ante fallos). Detecta configuraciones inseguras.',
        shortDesc: 'Auditor├¡a de seguridad de email',
        icon: 'fas fa-key',
        endpoint: null,
        form: {
      ...
      },
      {
        ...
        description: 'Enumera subdominios de un dominio objetivo usando t├®cnicas pasivas (Certificate Transparency, DNS brute-force). Descubre paneles de administraci├│n, entornos de staging o servicios no documentados.',
        shortDesc: 'Enumeraci├│n de subdominios',
        icon: 'fas fa-sitemap',
        endpoint: null,
        form: {
    ...
    tools: [
      {
        ...
        description: 'Busca un nombre de usuario en m├ís de 300 plataformas simult├íneamente: Twitter/X, Instagram, LinkedIn, GitHub, Reddit, TikTok y m├ís. Construye el perfil digital y footprint de una persona u organizaci├│n.',
        shortDesc: 'B├║squeda en 300+ plataformas',
      ...
      {
        name: 'Phone Lookup',
        description: 'Investiga un n├║mero de tel├®fono para obtener el operador, pa├¡s de origen, tipo de l├¡nea (m├│vil/fijo/VoIP), posible nombre del titular y si aparece en directorios p├║blicos o bases de datos filtradas.',
        shortDesc: 'Investigaci├│n de n├║meros telef├│nicos',
        icon: 'fas fa-phone',
        endpoint: null,
        form: {
          title: 'Phone Lookup',
          fields: [
            { name: 'phone', type: 'tel', label: 'Tel├®fono', placeholder: '+34 600 000 000', required: true }
          ]
        }
      },
      {
        name: 'Image Reverse',
        description: 'B├║squeda inversa de im├ígenes usando Google Lens, Yandex y TinEye en paralelo. Localiza el origen de una imagen, detecta copias, deepfakes o el contexto real de una fotograf├¡a.',
        shortDesc: 'B├║squeda inversa de im├ígenes',
        icon: 'fas fa-image',
        endpoint: null,
        form: {
      ...
      {
        name: 'Paste Search',
        description: 'Busca en Pastebin, GitHub Gists, Hastebin y otros servicios de pasta texto. Detecta credenciales, tokens API, c├│digo fuente o datos personales filtrados relacionados con tu objetivo.',
        shortDesc: 'B├║squeda en sitios de paste p├║blico',
        icon: 'fas fa-paste',
        endpoint: null,
        form: {
          title: 'Paste Search',
          fields: [
            { name: 'query', type: 'text', label: 'T├®rmino de B├║squeda', placeholder: 'email, dominio o keyword', required: true }
          ]
        }
      },
      {
        name: 'Profile Analyzer',
        description: 'Analiza un perfil de red social y extrae metadatos: horarios de actividad, an├ílisis de sentimiento, relaciones, idioma, geolocalizaci├│n impl├¡cita y posibles seud├│nimos o cuentas vinculadas.',
        shortDesc: 'An├ílisis profundo de perfiles sociales',
  ...
  'An├ílisis Forense': {
    ...
    description: 'Herramientas de an├ílisis forense digital',
    tools: [
      {
        name: 'Hash Analyzer',
        description: 'Analiza un hash MD5, SHA-1, SHA-256 o SHA-512 contra la base de datos de VirusTotal y Malware Bazaar. Determina si corresponde a un archivo malicioso conocido y obt├®n su historial de detecciones.',
        shortDesc: 'Verificaci├│n de hash en VirusTotal',
      ...
      {
        name: 'URL Scanner',
        description: 'Analiza una URL en m├ís de 70 motores antivirus y servicios de inteligencia de amenazas. Detecta phishing, malware drive-by, redirectores maliciosos y calcula una puntuaci├│n de riesgo global.',
        shortDesc: 'An├ílisis de URLs en 70+ motores',
        icon: 'fas fa-link',
        endpoint: 'virustotal',
        form: {
          title: 'URL Scanner',
      ...
      },
      {
        ...
        description: 'Extrae metadatos EXIF de im├ígenes JPEG/PNG: coordenadas GPS, c├ímara utilizada, fecha y hora, software de edici├│n. Tambi├®n analiza metadatos ocultos en documentos PDF y Office.',
        shortDesc: 'Extracci├│n de metadatos EXIF/PDF',
      ...
      },
      {
        ...
        description: 'Comprueba si una IP est├í listada en m├ís de 100 listas negras RBL/DNSBL (Spamhaus, Barracuda, SORBS...). Detecta si una IP est├í siendo usada para spam, ataques DDoS o botnets activas.',
        shortDesc: 'Verificaci├│n en 100+ blacklists',
        icon: 'fas fa-ban',
        endpoint: null,
        form: {
          title: 'IP Blacklist Check',
          fields: [
            { name: 'ip', type: 'text', label: 'Direcci├│n IP', placeholder: '192.168.1.1', required: true }
          ]
        }
      },
      {
        name: 'HTTP Headers',
        description: 'Analiza las cabeceras HTTP de respuesta de un servidor web. Detecta tecnolog├¡as utilizadas (Server, X-Powered-By), presencia de cabeceras de seguridad (CSP, HSTS, X-Frame-Options) y posibles fugas de informaci├│n.',
        shortDesc: 'An├ílisis de cabeceras HTTP del servidor',
        icon: 'fas fa-code',
        endpoint: null,
        form: {
          title: 'HTTP Headers',
...
document.addEventListener('DOMContentLoaded', function () {
  console.log('­ƒÜÇ Initializing OSINT AI Pro Platform...');
  ...
  try {
    ...
    console.log('Ô£à OSINT AI Pro Platform initialized successfully');
  } catch (error) {
    console.error('ÔØî Initialization error:', error);
    showNotification('ÔÜá´©Å Plataforma iniciada en modo b├ísico', 'warning', 3000);
  }
});
...
function initializeLogin() {
  ...
  if (loginBtn) {
    loginBtn.addEventListener('click', function () {
      ...
      if (!email || !password) {
        showNotification('ÔÜá´©Å Por favor ingresa email y contrase├▒a', 'warning');
        return;
      }
      ...
      if (email === applicationData.user_credentials.email && password === applicationData.user_credentials.password) {
        ...
        showNotification('­ƒöÉ Verificando credenciales...', 'info');
        ...
        setTimeout(() => {
          ...
          showNotification('Ô£à Acceso autorizado. Bienvenido a OSINT AI Pro', 'success');
        ...
        showNotification('ÔØî Credenciales incorrectas', 'error');
      }
    });
  }
...
function logout() {
  ...
  showNotification('­ƒæï Sesi├│n cerrada correctamente', 'info');
}
...
function initializeSidebar() {
  ...
  if (langSelect) {
    langSelect.addEventListener('change', function (e) {
      ...
      showNotification(newLang === 'es' ? '­ƒç¬­ƒç© Idioma cambiado a Espa├▒ol' : '­ƒç║­ƒç© Language changed to English', 'info');
...
function initializeDashboard() {
  ...
  // Esperar a que el DOM est├® 100% listo
  setTimeout(() => {
    initializeClickableMetrics();
    console.log('Ô£à Dashboard listeners initialized');
  }, 500);
  ...
  setTimeout(() => {
    if (OSINTApp.isLoggedIn) {
      initializeKasperskyMap();
      startLiveMetrics(); // <- Iniciar m├®tricas en vivo
    }
  }, 1000);
}
...
function initializeHeaderButtons() {
  ...
  if (quickScanBtn) {
    quickScanBtn.addEventListener('click', (e) => {
      ...
      if (searchInput) {
        // Asignar IP default para escaneo r├ípido si est├í vac├¡o
        if (!searchInput.value) searchInput.value = '127.0.0.1';
      }

      // Ir directo a la secci├│n de Inteligencia donde ocurre el an├ílisis
...
function startLiveMetrics() {
  // Actualizaci├│n aleatoria de las m├®tricas cada 5-10 segundos
  setInterval(() => {
...
function initializeClickableMetrics() {
  console.log('­ƒöì Inicializando m├®tricas clickables...');

  setTimeout(() => {
    const metricsGrid = document.querySelector('.metrics-grid');
    if (!metricsGrid) {
      console.error('ÔØî NO SE ENCONTR├ô CONTENEDOR DE M├ëTRICAS');
      return;
    }
    ...
    metricsGrid.addEventListener('click', function (e) {
      ...
      console.log('­ƒû▒´©Å CLICK en m├®trica:', modalId);
      openModal(modalId);

      setTimeout(() => {
        populateDetailModal(modalId);
        console.log('­ƒôè Gr├íficos cargados en modal:', modalId);
      }, 200);
    });

    console.log('Ô£à Listeners de delegaci├│n agregados a m├®tricas');
  }, 1000);
}
...
function initializeThreatAlert() {
  ...
  if (threatAlert) {
    threatAlert.addEventListener('click', function () {
      showNotification('­ƒöì Desplazando al Panel de Amenazas', 'info');
...
function initializeNotificationsButton() {
  ...
  if (markAllReadBtn) {
    markAllReadBtn.addEventListener('click', function () {
      showNotification('Ô£à Notificaciones marcadas como le├¡das', 'success');
...
function populateNotificationsModal(clear = false) {
  ...
  const notifs = [
    { id: 1, type: 'critical', title: 'Alerta Cr├¡tica: Filtraci├│n detectada', desc: 'Se ha comprometido una cuenta de administrador en un servicio conectado.', time: 'Hace 5 min', icon: 'fa-shield-alt', color: '#ef4444' },
    { id: 2, type: 'warning', title: 'Actualizaci├│n pendiente', desc: 'La API de Shodan requiere renovaci├│n de token.', time: 'Hace 2 horas', icon: 'fa-exclamation-triangle', color: '#f59e0b' },
    { id: 3, type: 'success', title: 'Reporte generado', desc: 'El reporte ejecutivo de seguridad est├í listo para descarga.', time: 'Ayer', icon: 'fa-file-pdf', color: '#10b981' }
  ];
...
function initializeKasperskyMap() {
  try {
    ...
    if (!mapContainer) {
      console.warn('ÔÜá´©Å Map container not found');
      return;
    }

    // Limpiar contenedor por si exist├¡a el mapa anterior de Leaflet
    mapContainer.innerHTML = '';

    // Check si Globe.gl est├í disponible
    if (typeof Globe === 'undefined') {
      console.error('Globe.gl no est├í cargado');
      return;
    }
    ...
    const threatLocations = [
      ...
      { lat: 55.7558, lng: 37.6176, city: 'Mosc├║', type: 'NAV' },
      { lat: 39.9042, lng: 116.4074, city: 'Beijing', type: 'IDS' },
      { lat: -23.5505, lng: -46.6333, city: 'S├úo Paulo', type: 'DDS' },
      { lat: 52.5200, lng: 13.4050, city: 'Berl├¡n', type: 'MAV' }
    ];
    ...
    window.threatMapInstance = Globe()
      ...
      .height(mapContainer.clientHeight || 650) // Fijar altura din├ímica al contenedor
    ...
    // Auto-rotaci├│n para darle ese feeling "Kaspersky"
    ...
    window.threatMapInstance.onGlobeClick(({ lat, lng }) => {
      const fakeCountries = ['Rusia', 'Estados Unidos', 'China', 'Ir├ín', 'Alemania', 'Brasil', 'India', 'Corea del Norte', 'Reino Unido', 'Francia'];
      ...
      if (modalBody && modalTitle) {
        modalTitle.innerHTML = `<i class="fas fa-satellite" style="color: #39ff14"></i> Telemetr├¡a de Ubicaci├│n`;
        modalBody.innerHTML = `
                <div style="padding: 20px; text-align: center;">
                    ...
                    <p style="color: #cbd5e1; font-size: 15px; margin-bottom: 15px;">Se ha detectado actividad inusual saliente desde este nodo geoespacial. El sistema OSINT AI est├í interceptando paquetes para determinar la naturaleza de la amenaza.</p>
                    <div style="margin-top: 15px; padding: 15px; background: rgba(57, 255, 20, 0.1); border-radius: 8px; border-left: 4px solid #39ff14;">
                        <i class="fas fa-radar" style="color: #39ff14;"></i> Iniciando an├ílisis profundo de red...
                    </div>
                </div>
            `;
        openModal('analysisModal');

        // Pausar rotaci├│n si estaba activa y acercar c├ímara
    ...
    if (liveCountryEl && liveThreatEl) {
      setInterval(() => {
        const countries = ['Rusia', 'EE.UU.', 'China', 'Brasil', 'India', 'Reino Unido', 'Alemania', 'Ir├ín', 'Sud├ífrica', 'Jap├│n', 'Australia'];
        const threats = [
          'Ataque DDoS en puerto 443', 'Escaneo masivo de puertos TCP',
          'Infecci├│n Botnet detectada', 'Exfiltraci├│n de datos sospechosa',
          'Intento de Login por Fuerza Bruta', 'Tr├ífico C&C identificado'
        ];
        ...
        // Peque├▒o efecto flash en la caja
    ...
    // A├▒adir ataques aleatorios continuamente
    OSINTApp.threatMapInterval = setInterval(() => {
      updateThreatCounts();

      // Lanzar un nuevo l├íser/arco
      const newArc = {
        startLat: (Math.random() - 0.5) * 180,
        startLng: (Math.random() - 0.5) * 360,
        endLat: (Math.random() - 0.5) * 180,
    ...
    if (playPauseBtn) {
      ...
      playPauseBtn.addEventListener('click', () => {
        ...
        showNotification(isRotating ? 'ÔûÂ´©Å Rotaci├│n del globo reanudada' : 'ÔÅ©´©Å Rotaci├│n del globo pausada', 'info');
      });
    }
    ...
    if (zoomGlobalBtn) {
      zoomGlobalBtn.addEventListener('click', () => {
        ...
        showNotification('­ƒîì Vista global restaurada', 'info');
      });
    }
    ...
    interactiveLegends.forEach(legend => {
      legend.addEventListener('click', () => {
        ...
        const descriptions = {
          ...
          'DDS': { title: 'DDoS Attack (DDS)', desc: 'Intentos de denegaci├│n de servicio distribuido. Busca saturar los recursos de red o servidores con tr├ífico malicioso masivo.' },
          'MAV': { title: 'Mail Anti-Virus (MAV)', desc: 'Detecta y bloquea malware, phishing y enlaces maliciosos presentes en el tr├ífico de correo electr├│nico entrante y saliente.' },
          'NAV': { title: 'Network Attack Validator (NAV)', desc: 'Analiza el tr├ífico a nivel de red para detectar patrones an├│malos o intentos de explotaci├│n de vulnerabilidades conocidas.' },
          'IDS': { title: 'Intrusion Detection System (IDS)', desc: 'Monitorea activamente la red en busca de actividades sospechosas, intentos de intrusi├│n y violaciones de pol├¡ticas.' }
        };
        ...
        if (type && descriptions[type]) {
          ...
          if (modalBody && modalTitle) {
            // Sacar el color visual de la leyenda para usarlo en el ├¡cono del modal
            ...
            modalBody.innerHTML = `
                    <div style="padding: 20px; text-align: center;">
                        ...
                        <div style="margin-top: 20px; padding: 15px; background: rgba(59, 130, 246, 0.1); border-radius: 8px; border-left: 4px solid #3b82f6;">
                            <i class="fas fa-info-circle"></i> La telemetr├¡a en tiempo real clasifica estos eventos de seguridad a nivel global.
                        </div>
                    </div>
                `;
            openModal('analysisModal');
          } else {
            showNotification(`Ôä╣´©Å ${info.title}: ${info.desc}`, 'info');
          }

          // Opcional: enfocar de forma m├ís sutil sin volverse loco
          if (window.threatMapInstance) {
    ...
    console.log('Ô£à Kaspersky 3D Map initialized (Globe.gl) with Modal prompts');

  } catch (error) {
    console.error('ÔØî Map initialization failed:', error);
  }
}

function showThreatDetails(threat, threatType) {
  // Reutilizaremos un modal gen├®rico o el modal de "Detalles" si lo creamos.
  // Para simplificar, insertamos din├ímicamente el contenido en el modal 'scoreModal' o similar,
  // o creamos uno din├ímico usando SweetAlert/HTML. 
  // Usaremos una alerta personalizada tipo notificaci├│n grande:
  const threatMsg = `
        <div style="background: #1a1a2e; border-left: 4px solid ${threatType.color}; padding: 15px; border-radius: 5px;">
            ...
            <p style="color: #cbd5e1;"><strong>Ubicaci├│n:</strong> ${threat.city}</p>
            <p style="color: #cbd5e1;"><strong>Severidad:</strong> Alta</p>
            <p style="color: #94a3b8; font-size: 11px; margin-top: 5px;">Identificador Autom├ítico del Sistema de Defensa Activa.</p>
        </div>
    `;

  // Inyectarlo en un contenedor global de informaci├│n si existe, o usar una notificaci├│n expandida
...
function addRandomThreat() {
  ...
  const locations = ['Nueva York', 'Londres', 'Tokio', 'Mosc├║', 'Beijing', 'S├úo Paulo', 'Berl├¡n', 'Par├¡s'];
  ...
  // A├▒adir evento click al item del feed
...
function initializeIntelligenceSearch() {
  ...
  if (searchBtn && searchInput) {
    searchBtn.addEventListener('click', () => {
      ...
      if (!target) {
        showNotification('ÔØî Por favor, introduce un objetivo v├ílido (IP, Dominio, Email...)', 'error');
        return;
      }
      ...
      const messages = [
        'Estableciendo t├║neles seguros y evadiendo firewalls...',
      ...
      setTimeout(() => {
        ...
        showNotification('Ô£à An├ílisis completado con ├®xito', 'success');
...
function populateIntelligenceResults(target) {
  ...
  // Generamos datos aleatorios cre├¡bles para The Threat
  ...
  let levelTxt = 'BAJO';
  let verdict = `El objetivo analizado (${target}) no muestra indicadores de compromiso en las fuentes p├║blicas. El tr├ífico asociado es benigno.`;
  ...
  if (riskScore >= 80) {
    ...
    riskDesc.textContent = "El objetivo est├í clasificado como una amenaza persistente avanzada (APT) o infraestructura maliciosa activa.";
    verdict = `ÔÜá´©Å ALERTA GRAVE: El modelo neuronal ha detectado que ${target} est├í listado en 14 repositorios de malware y fue usado en una campa├▒a de Ransomware recientemente. A├¡sle cualquier sistema conectado a este nodo inmediatamente.`;
    ...
    riskDesc.textContent = "Actividad sospechosa detectada. El objetivo podr├¡a estar involucrado en escaneos masivos o distribuci├│n de Adware.";
    verdict = `El objetivo presenta un comportamiento inusual. Se recomienda a├▒adir ${target} a la lista de monitoreo o listas grises preventivas.`;
    ...
    riskDesc.textContent = "Sin indicios de actividad maliciosa. Reputaci├│n limpia seg├║n los principales vendors de seguridad.";
    document.getElementById('dataDarkWeb').className = "dark-web-alert info";
  ...
  if (!isIp) {
    ...
    document.getElementById('dataNames').innerHTML = `<li>No hay resoluci├│n DNS / CNAME pasiva.</li>`;
  }
  ...
  OSINTApp.searchResults = {
    ...
    findings: [
      ...
      { tool: 'CVE Analysis', result: riskScore >= 60 ? 'CVE-2021-34527 detectado' : 'No se detectaron vulnerabilidades cr├¡ticas', status: riskScore >= 60 ? 'danger' : 'success', raw: riskScore >= 60 ? 'CVSS 8.8 (PrintNightmare)' : 'Clean scan' },
...
function initializeExportButtons() {
  ...
  if (btnPdf) {
    btnPdf.addEventListener('click', () => {
      showNotification('­ƒôä Generando PDF premium...', 'info');
      ...
      setTimeout(async () => {
        try {
          ...
          showNotification('Ô£à PDF generado con ├®xito', 'success');
        } catch (err) {
          console.error('Error generando PDF:', err);
          showNotification('ÔØî Error al generar PDF. Usando impresi├│n nativa.', 'error');
          window.print();
        }
      }, 500);
    });
  ...
  if (btnSave) {
    btnSave.addEventListener('click', () => {
      if (!OSINTApp.searchResults) {
        showNotification('ÔÜá´©Å No hay resultados para guardar', 'warning');
        return;
      }

      const reportName = prompt('Nombre para el reporte:', `Investigaci├│n ${OSINTApp.searchResults.target}`);
      ...
      showNotification('Ô£à Reporte guardado correctamente en la secci├│n Reportes', 'success');
...
function exportSearchResults(format) {
  if (!OSINTApp.searchResults) {
    showNotification('ÔÜá´©Å No hay resultados para exportar', 'warning');
    return;
  }
  ...
  if (format === 'json') {
    ...
    showNotification('Ô£à Resultados exportados en JSON', 'success');
  }
}
...
function executeNetworkAnalysis() {
  ...
  // Navegar a inteligencia si no estamos ah├¡
  if (intelligenceNav) intelligenceNav.click();

  showNotification('­ƒîÉ Detectando IP p├║blica...', 'info');
  ...
  fetch('https://api.ipify.org?format=json')
    ...
    .then(data => {
      if (targetInput) {
        ...
        showNotification('­ƒîÉ IP P├║blica detectada: ' + data.ip, 'info');

        // Ejecutar b├║squeda autom├íticamente
        const searchBtn = document.getElementById('startUniversalSearchBtn');
        if (searchBtn) {
    ...
    .catch(() => {
      if (targetInput) {
        ...
        showNotification('­ƒîÉ Error detectando IP p├║blica. Usando localhost.', 'warning');
        const searchBtn = document.getElementById('startUniversalSearchBtn');
...
function initializeThemeToggle() {
  ...
  themeBtn.addEventListener('click', () => {
    ...
    showNotification(`­ƒîù Modo ${newTheme === 'dark' ? 'Oscuro' : 'Claro'} activado`, 'info');
  });
}
...
// TOOLS SECTION ÔÇö v2.0 (inline panel, toggles, search)
...
function renderToolsGrid(container, enabledTools, searchFilter = '') {
  ...
  container.querySelectorAll('.tool-toggle-input').forEach(input => {
    input.addEventListener('change', function () {
      ...
      updateToolsStats();
      showNotification(
        this.checked ? `Ô£à ${toolName} activada` : `ÔÜ½ ${toolName} desactivada`,
...
// ÔöÇÔöÇ INLINE EXECUTION PANEL ÔöÇÔöÇ
function openInlineToolPanel(toolData) {
  ...
  // Reset results
  if (resultsArea) resultsArea.innerHTML = '<div class="inline-results-placeholder"><i class="fas fa-terminal"></i><p>Esperando par├ímetros...</p></div>';
...
async function executeInlineTool(toolData) {
  ...
  if (!form || !form.checkValidity()) {
    showNotification('ÔÜá´©Å Completa los campos requeridos', 'warning');
  ...
  container.innerHTML = `
    <div class="inline-result-success">
      <div class="inline-result-header">
        ...
        <span>${toolData.name} ÔÇö Resultado</span>
...
function copyResultToClipboard(btn) {
  ...
  navigator.clipboard.writeText(JSON.stringify(JSON.parse(data), null, 2)).then(() => {
    showNotification('­ƒôï Resultado copiado al portapapeles', 'success');
  });
}
...
function initializeReportsSection() {
  ...
  // Si existe un bot├│n de "Generar Reporte" global (por ejemplo de Investigaci├│n Inteligente)
  ...
  if (generateReportBtn) {
    ...
    newBtn.addEventListener('click', function () {
      ...
      if (!reportName) {
        showNotification('ÔÜá´©Å Ingresa nombre del reporte', 'warning');
        return;
      }
      ...
      showNotification(`Ô£à Reporte "${reportName}" generado y guardado localmente`, 'success');
      closeAllModals();
...
function renderReportsList() {
  ...
  if (OSINTApp.reports.length === 0) {
    container.innerHTML = '<p style="color: #94a3b8; grid-column: 1/-1;">No tienes reportes guardados todav├¡a.</p>';
    return;
  }
  ...
  container.innerHTML = OSINTApp.reports.map((report, index) => `
        <div class="report-card" style="display: flex; flex-direction: column; justify-content: space-between; border-left: 4px solid var(--color-primary); background: rgba(30, 41, 59, 0.4); border-radius: 12px; padding: 18px; transition: all 0.3s ease;">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                <button class="btn btn--primary btn--sm" onclick="viewReportDetails(${index})" style="flex: 1 1 100%; border-radius: 6px; padding: 10px; font-weight: 500;"><i class="fas fa-eye"></i> Ver An├ílisis</button>
...
window.viewReportDetails = function (index) {
  ...
  if (!report || !report.data) {
    showNotification('ÔÜá´©Å Este reporte no contiene datos de an├ílisis.', 'warning');
    return;
  }
          ...
          <div style="padding: 5px;">
              <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; background: linear-gradient(135deg, rgba(56, 189, 248, 0.15) 0%, rgba(56, 189, 248, 0.05) 100%); padding: 18px; border-radius: 12px; border: 1px solid rgba(56, 189, 248, 0.3);">
                  ...
                  <div style="text-align: right;">
                      <div style="font-size: 32px; font-weight: 800; color: ${data.riskLevel === 'CR├ìTICO' ? '#ef4444' : data.riskLevel.includes('ALTO') ? '#f59e0b' : '#10b981'}; text-shadow: 0 0 15px rgba(0,0,0,0.5);">${data.riskScore}%</div>
                      <div style="font-size: 10px; color: #94a3b8; font-weight: bold; letter-spacing: 1px;">RISK SCORE</div>
                      <div style="font-size: 12px; margin-top: 5px; color: ${data.riskLevel === 'CR├ìTICO' ? '#ef4444' : data.riskLevel.includes('ALTO') ? '#f59e0b' : '#10b981'}; font-weight: bold;">${data.riskLevel}</div>
                  </div>
              </div>
              ...
              <div style="margin-bottom: 25px; padding: 15px; background: rgba(0,0,0,0.2); border-radius: 8px; border: 1px dashed rgba(255,255,255,0.1);">
                  <h4 style="color: #38bdf8; margin-bottom: 8px; font-size: 14px; text-transform: uppercase;">Conclusi├│n de la IA:</h4>
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                  <div style="background: rgba(30, 41, 59, 0.4); padding: 12px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.05);">
                      <div style="color: #94a3b8; font-size: 10px; text-transform: uppercase; margin-bottom: 4px;">Ubicaci├│n Detectada</div>
...
window.exportReport = async function (index, format) {
  ...
  if (format === 'json') {
    // Si exportas a JSON, ahora incluir├í el campo aiAnalysis si existe
    const dataToExport = { ...report, exportedAt: new Date().toISOString() };
    const blob = new Blob([JSON.stringify(dataToExport, null, 2)], { type: 'application/json' });
    downloadFile(blob, `report-${report.id}.json`);
    showNotification(`Ô£à Reporte JSON exportado`, 'success');
  } else if (format === 'pdf') {
    showNotification('­ƒôä Generando PDF con Inteligencia Forense...', 'info');
    ...
    setTimeout(async () => {
      ...
      try {
        const { jsPDF } = window.jspdf;
        // Capturamos el modal incluyendo la nueva secci├│n de IA
        const canvas = await html2canvas(element, {
        ...
        console.error('Error generando PDF:', err);
        showNotification('ÔØî Error al generar PDF.', 'error');
      }
    }, 1000); // Damos un poco m├ís de tiempo para que la IA se renderice bien
  }
};
...
window.deleteReport = function (index) {
  if (confirm('┬┐Seguro que deseas eliminar este reporte permanentemente?')) {
    ...
    localStorage.setItem('osint_reports', JSON.stringify(OSINTApp.reports));
    renderReportsList();
    showNotification('­ƒùæ´©Å Reporte eliminado', 'info');
  }
};
...
// ==========================================
// SECCI├ôN DE MONITOREO VIG├ìA IA
// ==========================================
...
function initializeMonitoringSection() {
  ...
  if (!localStorage.getItem('osint_monitoring')) {
    OSINTApp.monitoring = [
      { id: 1, name: 'empresa-target.com', status: 'active', threatLevel: 'success', lastCheck: 'hace 5 min', aiInsight: 'Ô£à Gemini: Per├¡metro seguro.' }
    ];
...
window.showAddMonitorModal = function() {
  ...
  const newTarget = {
    ...
    aiInsight: '­ƒñû Gemini iniciando rastreo...'
  };
...
function simulateAIWatcher(id) {
  setTimeout(() => {
    ...
    OSINTApp.monitoring[idx].aiInsight = isThreat ? "ÔÜá´©Å Alerta IA: Tr├ífico an├│malo detectado." : "Ô£à Gemini: Activo seguro.";
    localStorage.setItem('osint_monitoring', JSON.stringify(OSINTApp.monitoring));
    renderMonitoringList();
    if(isThreat) showNotification('­ƒÜ¿ Amenaza en el Vig├¡a', 'error');
  }, 4000);
}
...
function initializeMonitoringCharts() {
  ...
  if (ctxPie) {
    new Chart(ctxPie, {
      type: 'doughnut',
      data: {
        labels: ['Europa', 'Am├®rica', 'Asia', '├üfrica', 'Ocean├¡a'],
        datasets: [{
          data: [35, 25, 20, 10, 10],
...
function setupMonitoringControls() {
  ...
  if (exportBtn) {
    exportBtn.onclick = () => {
      showNotification('­ƒôÑ Exportando JSON...', 'success');
    };
  }
...
console.log("­ƒÜÇ OSINT AI Pro: Despegue completado. Sistema al 100%.");
